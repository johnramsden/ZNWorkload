...
# Create a block device (optionally ZNS) using configfs
# REF: https://zonedstorage.io/docs/getting-started/zbd-emulation

if [ $# != 5 ]; then
        echo "Usage: $0 <sect size (B)> <zone size (MB)> <nr conv zones> <nr seq zones> <zns|block>"
        exit 1
fi
